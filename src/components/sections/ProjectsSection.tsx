  ...
  return (
    <section className="bg-white py-24 lg:py-28">
      <div className="mx-auto max-w-7xl px-6 lg:px-8 flex flex-col gap-[72px]">
        ...
        {/* Cards — subgrid para alinear filas entre columnas en desktop */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:[grid-template-rows:repeat(6,auto)]">
          {projects.map((p) => (
            <div
              key={p.slug}
              className="rounded-[24px] p-12 grid gap-6 lg:[grid-template-rows:subgrid] lg:[grid-row:span_6]"
              style={{ backgroundColor: "#f8f7f2" }}
            >
              {/* Header: logotipo + isotipo */}
              <div className="relative flex items-start w-full h-[61px]">
              ...
              {/* Descripción */}
              <p className="font-sans text-[18px] font-normal leading-[1.6] text-text-secondary text-justify">
                {p.description}
              </p>

              {/* Imagen */}
              <div className="relative h-[350px] rounded-[20px] overflow-hidden">
